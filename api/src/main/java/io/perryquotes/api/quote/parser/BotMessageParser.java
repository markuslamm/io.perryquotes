...
@Component
public class BotMessageParser extends LoggableComponent {
  ...
  public ParserResult parse(final String input) {
    ...
    if(authors.isEmpty()) {
      throw new InvalidDataException("Invalid data: No authors provided");
    }
    log.debug("Parsed authors: {}", authors);

    var bookSource= parseBookSource(input).orElseThrow(
      () -> new InvalidDataException("Invalid data: No book source provided"));
